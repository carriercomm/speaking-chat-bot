...
function execute_bot() {
    ...
    /**
     * Speak out the message sent
     * @param {Object} request
     */
    add_command('say', function(request) {
          say.speak('Pipe Organ', request.stanza.getChildText('body').trim().replace("say;",""), function () {
            sys.puts('text to speech complete'.green);
          });     

        //  translate.text({input:'English',output:'Hindi'},request.stanza.getChildText('body').trim().replace("say;",""), function(result){
        //        say.speak('Alex', result);
        //});  
        return true;
    });    

    add_command('wiki', function(request) {
        // shttp://www.google.com/search?q=node.js+site%3Awikipedia.org 

        var wikipedia = require("wikipedia-js");
        var query = request.stanza.getChildText('body').trim().replace("wiki;","");
        ...
        var wikia = htmlWikiText.replace(/<(?:.|\n)*?>/gm, ' ');
        say.speak('Agnes', wikia, function () {
            sys.puts('text to speech complete'.green);
          });
        });
